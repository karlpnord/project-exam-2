...
const AllVenues = () => {
	...
	const { data, isLoading, isSuccess, fetchNextPage, hasNextPage, isFetchingNextPage } = useInfiniteVenues();

	const lastVenueElementRef = useObserverInfiniteScrolling(isLoading, isFetchingNextPage, hasNextPage, fetchNextPage);
			...
			<div className='bg-whiteBg font-inter'>
				<MaxWidthWrapper className='relative z-20 pb-12 pt-24 md:pb-36 md:pt-36'>
					<div className='px-2 mb-10 flex flex-col sm:flex-row sm:items-center justify-between'>
						<h1 className='text-4xl mb-2 font-bold text-textDark'>All Venues</h1>
						<SearchForm setSearchValue={setSearchValue} searchValue={searchValue} />
					</div>
					{isLoading && <Loader />}
					{isSuccess && <VenueList venues={filteredVenues} lastVenueElementRef={lastVenueElementRef} />}
