...
const Question = ({ id, title, children, handler, openId }: QuestionsProps) => {
	...
	return (
		<motion.div animate={openId === id ? 'open' : 'closed'} className='border-b-[1px] border-borderClr font-inter'>
			<button onClick={() => handler(id)} className='flex w-full items-center justify-between gap-4 py-6'>
				<motion.span
					variants={{
						open: {
							color: '#6633DE',
