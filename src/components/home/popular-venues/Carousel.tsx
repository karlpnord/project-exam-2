...
const Carousel = () => {
	...
	const { data, isLoading, isSuccess } = usePosts(`${apiBaseUrl}/venues?_owner=true`);
	...
	const CARD_BUFFER = width > BREAKPOINTS.lg ? 4 : width > BREAKPOINTS.sm ? 2 : 1;

	const CAN_SHIFT_LEFT = offset < 0;

	const CAN_SHIFT_RIGHT = Math.abs(offset) < CARD_SIZE * (CARDS_AMOUNT - CARD_BUFFER);
	...
	return (
		<section className='relative overflow-hidden bg-defaultBg py-24 font-inter' ref={ref}>
